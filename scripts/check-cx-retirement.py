...
def main():
    with tempfile.TemporaryDirectory(prefix='cx-retirement-') as directory:
        ...
        for scenario in ('both','disabled','masked','system-dirs','lab-residual'):
            output = stage/scenario; output.mkdir()
            result = subprocess.run(['bash',str(fixture/'run.sh'),scenario,
                str(stage/'new.deb'),str(stage/'old-mesh.deb'),str(stage/'historical.deb'),str(stage/'old1.deb'),str(stage/'old6.deb'),str(stage/'baseline.deb')],
            ...
            assert receipt['native_chain'] == (['0.3.1-4','0.3.3-1','0.3.3-6','cx-mesh1.1.0-1','cx-mesh1.2.0-1'] if scenario=='lab-residual' else ['0.3.1-4','0.3.3-1','cx-mesh1.2.0-1'])
            assert len(receipt['denied_before_dpkg']) == (2 if scenario=='lab-residual' else 8)
            if scenario in ('system-dirs','lab-residual'):
                assert receipt['residual_list_sha256']=='a23ac038fce0108c1af26c8542cc7b2445fbd76c1da22b5fb725dd0cbcca9600'
            assert receipt['no_purge'] and receipt['config_identity_receipt_session_full_metadata_preserved']
            print('CX retirement passed: '+scenario+'; actual APT/DPKG, production guard, retained configuration, repeat install and refusal checks',flush=True)
