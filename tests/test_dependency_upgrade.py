    ...
    def plan(self,missing=None,legacy=False):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fields(name,version):
                ...
                if name=='agent-sphere':data['Depends']=', '.join(f'{n} (>= {v})' for n,v in DEPS.items()) if version=='0.2.0-6' else 'medge (>= 3.0.0-3)'
            ...
            for n,v in [*DEPS.items(),('agent-sphere','0.2.0-6'),('agpc-manager','3.1.0-2'),('medge','3.1.0-2'),*existing]:
            ...
            result=subprocess.run([*command,'--simulate','--no-remove','install','agent-sphere=0.2.0-6'],capture_output=True,text=True,env=env)
