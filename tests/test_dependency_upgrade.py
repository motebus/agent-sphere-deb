...
TRANSITIVE={'uchatd':'0.5.0-1','redis-server':'5:7.0','mote-transportd':CORE['mote-transportd']}


class CoreDependencyTests(unittest.TestCase):
    def plan(self,missing=None,legacy=False,old_cx=False):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fields(name,version):
                ...
                if name=='agent-sphere':data['Depends']=', '.join(f'{n} (>= {v})' for n,v in DEPS.items()) if version=='0.3.0-33' else 'medge (>= 3.0.0-3)'
                ...
                if name=='agpc-manager':data['Depends']='medge (>= 3.3.0-1)'
                return data
            existing=[('agent-sphere','0.1.0-12'),('medge','3.0.0-3')] if legacy else []
            ...
            for n,v in [*{**DEPS,**TRANSITIVE}.items(),('agent-sphere','0.3.0-33'),('agpc-manager','3.3.0-1'),('medge','3.3.0-1'),*existing]:
            ...
            result=subprocess.run([*command,'--simulate','--no-remove','install','agent-sphere=0.3.0-33'],capture_output=True,text=True,env=env)
    ...
    def test_missing_contextd_does_not_allow_partial_core(self):
        result=self.plan(missing='contextd');self.assertNotEqual(result.returncode,0,result.stdout+result.stderr)

    def test_existing_consolidated_cx_is_upgraded_with_new_core(self):
        result=self.plan(old_cx=True)
        self.assertEqual(result.returncode,0,result.stdout+result.stderr)
        self.assertRegex(result.stdout,r'Inst cx-mesh \[1\.2\.0-1\] \(2\.0\.0-1 ')
