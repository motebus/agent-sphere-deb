    ...
    def plan(self,missing=None,legacy=False,old_cx=False):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fields(name,version):
                ...
                if name=='agent-sphere':data['Depends']=', '.join(f'{n} (>= {v})' for n,v in DEPS.items()) if version=='0.3.0-8' else 'medge (>= 3.0.0-3)'
            ...
            for n,v in [*{**DEPS,**TRANSITIVE}.items(),('agent-sphere','0.3.0-8'),('agpc-manager','3.3.0-1'),('medge','3.3.0-1'),*existing]:
            ...
            result=subprocess.run([*command,'--simulate','--no-remove','install','agent-sphere=0.3.0-8'],capture_output=True,text=True,env=env)
