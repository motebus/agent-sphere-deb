...
""")
    ...
    def test_old_uchat_packages_refused_under_apt_lock(self):
        for name,version in [('uchat','3.2.0-1'),('uchatd','0.3.0-1')]:
            with self.subTest(name=name):
    ...
    def test_default_requires_confirmation_before_noninteractive_worker(self):
        ...
        self.assertEqual(self.calls()[1][:6], ['--simulate','install','agent-sphere=0.2.0-13','agent-ultra=0.1.0-1','agpc-manager=3.3.0-1','agent-apps=0.2.0-3'])
