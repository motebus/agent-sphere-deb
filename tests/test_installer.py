...
if os.environ.get('APT_TEST_FAIL') == stage:
    sys.exit(42)
if stage == 'simulate':
    print(os.environ.get('APT_TEST_PLAN', 'Inst agent-sphere (0.3.0-25 stable)\\nInst contextd (0.1.0-22 stable)'))
if stage == 'install':
...
""")
    ...
    def test_standard_requires_contextd_and_preserves_apps_without_selecting_them(self):
        ...
        for args in self.calls()[1:]:
            self.assertIn('contextd=0.1.0-22',args)
    ...
    def test_default_requires_confirmation_before_noninteractive_worker(self):
        ...
        self.assertEqual(self.calls()[1][:7], ['--simulate','install','agent-sphere=0.3.0-25','agent-ultra=0.1.0-1','agpc-manager=3.3.0-1','contextd=0.1.0-22','uchatd=0.5.0-1'])
