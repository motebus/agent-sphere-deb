...
class PackageTests(unittest.TestCase):
    ...
    def test_core_excludes_manager_ui_and_local_ultra(self):
        self.assertEqual(len(package.NAMES), 15)
        self.assertTrue({"agos", "model-router", "model-llm", "cx-mesh", "contextd", "uchatd", "cx-loop", "mote-mcpd", "mote-secd"}.issubset(package.NAMES))
        self.assertFalse({"medge", "sphere-manager", "agpc-manager", "agent-apps", "agent-ultra", "mdesk", "ss-webos", "obsidian", "mote-mcp-ultra"}.intersection(package.NAMES))
